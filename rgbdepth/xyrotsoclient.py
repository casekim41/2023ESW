...
camera_x_min, camera_x_max = -170, 210
arm_x_min, arm_x_max = -171, 220
camera_x_offset, arm_x_offset = 3, 0

camera_y_min, camera_y_max = 46, 200
arm_y_min, arm_y_max = 100, 250


HOST = '192.168.0.14'  # Use your server's IP address or hostname
...
def transform_point(x, y):
   ...
   transformed_x = ((x + camera_x_offset - camera_x_min) / (camera_x_max - camera_x_min)) * (arm_x_max - arm_x_min) + arm_x_min
   
   transformed_y = (y - camera_y_min) * (arm_y_max - arm_y_min) / (camera_y_max - camera_y_min) + arm_y_min    
   return transformed_x, transformed_y


def mouse_callback(event, x, y, flags, param):
    global goal_object_contour, goal_object_size_mm, goal_object_centroid, rotation_angle_deg
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if goal_object_centroid is not None : 
          new_center_x = goal_object_centroid[0] - new_origin_x
          new_center_y = goal_object_centroid[1] - new_origin_y  # Invert y-coordinate
          transformed_x, transformed_y = transform_point(new_center_x, new_center_y)
          intx, inty = int(transformed_x), int(transformed_y)
          print("Transformed coordinates (x, y):", int(transformed_x), int(transformed_y))
          data = f"{transformed_y} {transformed_x}" + ' 180 180 0 -'+ f"{rotation_angle_deg}"+ ' 14 30' 
          data = data.encode()
          length = len(data)
          client_socket.sendall(length.to_bytes(4, byteorder="little"))
          client_socket.sendall(data)
          print('sent',data)
          print("Mouse Callback - Rotation Angle:", rotation_angle_deg)
...
#############################################
       ...
       end_points = [(-196, 46), (210, 46), (-7, 202), (-147, 147),(133, 142)  ]
       new_points_sorted = sorted(end_points, key=lambda p: p[0])
       newend_points = [(x + new_origin_x, y + new_origin_y) for x, y in new_points_sorted]
       mask = np.zeros_like(newColorData)
       cv2.fillPoly(mask, np.int32([newend_points]), (255, 0, 255))
       #cv2.polylines(newColorData, [np.int32(newend_points)], isClosed=True, color=(0, 255, 255), thickness=2)
       alpha = 0.5
       newColorData = cv2.addWeighted(newColorData, 1, mask, alpha, 0)
       for contour in contours:
         if contour is not goal_object_contour and cv2.contourArea(contour) > 100 : 
             ...
             #update_rotation_angle(rotation_angle_deg)
